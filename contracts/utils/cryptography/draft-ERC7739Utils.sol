...
library ERC7739Utils {
     ...
     *
     * `signature ‖ DOMAIN_SEPARATOR ‖ contentsHash ‖ contentsDescr ‖ uint16(domainOffset) || uint16(contentsDescr.length)`
     *
        ...
        return
            keccak256(
                abi.encodePacked(
                    "TypedDataSign(",
                    contentsTypeName,
                    " contents,EIP712Domain signerDomain)",
                    contentsAndDomainType
                )
            );
    }

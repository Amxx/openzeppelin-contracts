...
export const product = (...arrays) => arrays.reduce((a, b) => a.flatMap(ai => b.map(bi => [...ai, bi])), [[]]);

// Range from start to end in increment
// Example: range(17,42,7) → [17,24,31,38]
export const range = (start, stop = undefined, step = 1) => {
  if (stop == undefined) {
    stop = start;
    start = 0;
  }
...
export const unique = (array, op = x => x) =>
...
export const mapValues = (obj, fn, fn2 = () => true) =>
  Object.fromEntries(
    Object.entries(obj)
      .filter(fn2)
      .map(([k, v]) => [k, fn(v)]),

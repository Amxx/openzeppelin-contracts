...
const overrideGetRootFilePaths = (
  hre: HardhatRuntimeEnvironment,
) => {
  const runSuper = hre.solidity.getRootFilePaths.bind(hre.solidity);
  const exposePath = getExposedPath(hre.config) + path.sep;

  return (options?: { scope?: BuildScope }) => runSuper(options).then(result => result.filter(p => !p.startsWith(exposePath)));
}

const overrideBuild = (
  hre: HardhatRuntimeEnvironment,
) => {
  const runSuper = hre.solidity.build.bind(hre.solidity);
  return async (
    ...
    switch (options?.scope) {
      case undefined:
      case "contracts":
        // Run first compilation run
        // can we use a different build profile here?
        const result = await runSuper(rootFilePaths, { ...options, quiet: true });

        // If new contract were compiled, expose them and add the exposed version to the input
        if (!(result instanceof Map) || result.size == 0) break;

        const buildId = await result.values().next().value!.compilationJob.getBuildId();
        const artefactPath = path.join(hre.config.paths.artifacts, "build-info", buildId + ".output.json");

        const fs = await import('fs/promises');
        const { output } = await fs.readFile(artefactPath, 'utf-8').then(JSON.parse);
    ...
    return runSuper(rootFilePaths, options);
  };
}
...
async function getMatcher(config: HardhatRuntimeEnvironment["config"]) {
  const path = await import('path');
  const { default: micromatch } = await import('micromatch');

  return (sourceName: string) : boolean =>
    sourceName.startsWith('project/') &&
    config.exposed.include.some(p => micromatch.isMatch(path.relative('project/', sourceName), p)) &&
    !config.exposed.exclude.some(p => micromatch.isMatch(path.relative('project/', sourceName), p));
}
...
import { formatLines, Lines, spaceBetween } from '../utils/format-lines.ts';


import { findAll, astDereferencer } from 'solidity-ast/utils.js';
import type { ASTDereferencer } from 'solidity-ast/utils';
...
export function getExposed(
  ...
  for (const { ast } of Object.values(solcOutput.sources)) {
    ...
    if (exposedFile !== undefined) {
      res.set(exposedFile.fsPath, exposedFile);
    }
...
function getExposedFile(
  ...
  const exposedPath = path.join(exposedRootPath, ...ast.absolutePath.startsWith('project/') ? [ path.relative('project', ast.absolutePath) ] : ['$_', ast.absolutePath]);
  const sourceName = path.relative(config.paths.root, exposedPath);
  ...
  return content === undefined
    ...
    : {
      ...
      inputSourceName: path.join('project', sourceName),
      fsPath: exposedPath,
      content,
      package: { name: '', version: '', rootFsPath: '', inputSourceNameRoot: '' } // TODO
    };
}
...
const overrideGetCompilationJobs = (
  hre: HardhatRuntimeEnvironment,
) => {
  const runSuper = hre.solidity.getCompilationJobs.bind(hre.solidity);
  return (
    rootFilePaths: string[],
    options?: GetCompilationJobsOptions,
  ) => {
    console.log("hre.solidity.getCompilationJobs called");
    return runSuper(rootFilePaths, options);
  };
}

const overrideRunCompilationJob = (
  hre: HardhatRuntimeEnvironment,
) => {
  const runSuper = hre.solidity.runCompilationJob.bind(hre.solidity);
  return (
    compilationJob: CompilationJob,
    options?: RunCompilationJobOptions,
  ) => {
    console.log("hre.solidity.runCompilationJob called");
    return runSuper(compilationJob, options).then(result => {
      console.log("hre.solidity.runCompilationJob answer");
      return result;
    });
  };
}
...
export default async (): Promise<Partial<HardhatRuntimeEnvironmentHooks>> => ({
  created: async (
    ...
    hre.solidity.getRootFilePaths = overrideGetRootFilePaths(hre);
    hre.solidity.build = overrideBuild(hre);
    hre.solidity.getCompilationJobs = overrideGetCompilationJobs(hre);
    hre.solidity.runCompilationJob = overrideRunCompilationJob(hre);
  }
});
